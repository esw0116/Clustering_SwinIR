...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        assert -self.window_size < self.shift_size < self.window_size, "shift_size must in 0-window_size"
        ...
        if self.shift_size > 0:
            attn_mask = self.calculate_mask(self.input_resolution, sign='plus')
        elif self.shift_size < 0:
            attn_mask = self.calculate_mask(self.input_resolution, sign='minus')
        else:
            attn_mask = None

        self.register_buffer("attn_mask", attn_mask)

    def calculate_mask(self, x_size, sign):
        ...
        if sign == 'plus':
            ...
            w_slices = (slice(self.window_size, None),
    ...
    def forward(self, x, x_size, **kwargs):
        ...
        if self.input_resolution == x_size:
            ...
            sign = 'plus' if self.shift_size > 0  else 'minus'
            if print_attn is True:
                attn_windows, attn_map = self.attn(x_windows, mask=self.calculate_mask(x_size, sign).to(x.device), **kwargs)
                ...
                attn_windows = self.attn(x_windows, mask=self.calculate_mask(x_size, sign).to(x.device), **kwargs)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, block_number,
        ...
        # build blocks
        shift_size_list = [0, window_size // 2] if block_number % 2 == 0 else [0, - window_size // 2]
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=shift_size_list[i%2],
                                 mlp_ratio=mlp_ratio,
