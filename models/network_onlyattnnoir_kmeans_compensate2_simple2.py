...
class ClusteredTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, keep_v=False, clustering=True, num_groups=16,
        ...
        self.compensate = nn.Linear(in_features=dim, out_features=dim)
        ...
        if clustering:
            self.kmeans_clustering = Clustering(self.num_groups, n_init=1)
    ...
    def forward(self, x, x_size, x_centers=None, labels=None, cnt_labels=None, **kwargs):
        ...
        if self.cluster_here:
            ...
            x_centers, labels = self.kmeans_clustering.fit(x_windows, enable_gradient=True)
        ...
        x_center_expand = x_centers.gather(dim=1, index=labels.view(*labels.size(), 1).repeat(1, 1, C))
        x_diff = x_windows - x_center_expand
        x_compensate = self.compensate(x_diff)
        attn_windows = attn_windows + x_compensate
            ...
            for j, blk in enumerate(self.blocks):
                ...
                if self.use_checkpoint:
                    ...
                    if self.shifted_window:
                        if j == 0:
                            x, x_centers1, labels1, cnt_labels1 = blk(x, x_size)
                        elif j == 1:
