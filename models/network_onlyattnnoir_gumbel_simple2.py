            ...
            for j, blk in enumerate(self.blocks):
                ...
                if self.use_checkpoint:
                    ...
                    if self.shifted_window:
                        if j == 0:
                            x, labels1, cnt_labels1 = blk(x, x_size)
                        elif j == 1:
                            x, labels2, cnt_labels2 = blk(x, x_size)
                        elif j % 2 == 0:
                            x, _, _ = blk(x, x_size, labels=labels1, cnt_labels=cnt_labels1, **kwargs)
                        else:
                            x, _, _ = blk(x, x_size, labels=labels2, cnt_labels=cnt_labels2, **kwargs)

                    else:
                        if j == 0:
                            x, labels, cnt_labels = blk(x, x_size, **kwargs)
                        else:
                            x, _, _ = blk(x, x_size, labels=labels, cnt_labels=cnt_labels, **kwargs)
        else:
            for j, blk in enumerate(self.blocks):
                timer_list = self.measure_time(msg=f'Block_{j}', timer_list=timer_list)
                if self.use_checkpoint:
