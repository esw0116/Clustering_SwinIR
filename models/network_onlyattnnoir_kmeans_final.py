...
class ClusteredTransformerBlock(nn.Module):
    ...
    '''
    def forward(self, x_centers, x_size, labels=None, cnt_labels=None, x_windows=None):
        H, W = x_size
        B, L, C = x_windows.shape
        print(x_windows.shape)
        shortcut = x_windows

        x_centers = self.norm1(x_centers)
        x_windows = self.norm1(x_windows)

        x_windows = x_windows.reshape(B, H, W, C)
        # # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x_windows, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x_windows
