...
class Mlp(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.fc1(x)
        if print_time:
            b = time.time(); print('Linear1:', b - a)
        x = self.act(x)
        if print_time:
            c = time.time(); print('GELU:', c - b)
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
        if print_time:
            d = time.time(); print('Linear2:', d - c)
        return x
